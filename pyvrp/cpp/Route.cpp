                        ...
                        std::vector<Duration> const &breakServices)
{
    ...
    for (auto it = activities.rbegin(); it != activities.rend(); ++it)
    {
        ...
        case Activity::ActivityType::CLIENT:
        {
            ...
            auto clientDS = DurationSegment(client);
            if (setup != 0)
    ...
    for (size_t tripIdx = 0; auto const &activity : activities)
    {
        ...
        case Activity::ActivityType::CLIENT:
        {
            ...
            Duration setup = 0;
            if (client.location != prevLoc)
                setup = data.setupDuration(client.location);

            handle(activity,
                   tripIdx,
                   client.twEarly,
                   client.twLate,
                   setup + client.serviceDuration);
...
void Route::setDistance(ProblemData const &data)
{
    ...
    for (size_t idx = 1; idx != schedule_.size(); ++idx)
    {
        ...
        auto toLoc = frmLoc;  // CUSTOM_BREAK: no travel
        switch (activity.type())
        {
        case Activity::ActivityType::DEPOT:
...
void Route::setLoad(ProblemData const &data)
{
    ...
    for (size_t dim = 0; dim != data.numLoadDimensions(); ++dim)
    {
        ...
        for (size_t idx = 1; idx != schedule_.size(); ++idx)
        {
            ...
            case Activity::ActivityType::CUSTOM_BREAK:
                break;  // breaks do not change load
            }
        }
