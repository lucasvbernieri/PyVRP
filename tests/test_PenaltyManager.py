...
def test_warn_when_violations_stuck(ok_small, recwarn):
    ...
    # Fork uses the 4-tuple penalty API (loads, tw, dist, breakDue); keep the
    # load penalty at max_penalty so the "stuck violation" warning fires.
    pm = PenaltyManager(([params.max_penalty], 1, 1, 1), params)
