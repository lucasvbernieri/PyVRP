...
| E | base `1607afc` | 228.2 | 128.1 | **0.561** |
| E | this branch, incl. the duration lower bound | 236.0 | **173.6** | **0.736** |
...
Do **not** expect this to be latency waste. **Five** independent experiments say
the break path is compute-bound, not memory-bound, once the heap traffic is gone
(the fifth being the cached-edge attempt above):
